<template>
    <div id="list">
        ...
        <el-dialog title="Delivering Coordinate" :visible.sync="viewLocVisible" width="70%">
            <BaiDuMapTrace  :markers="beView"></BaiDuMapTrace>
        </el-dialog>
        <h1 class="title">Delivering Medical Resource ：</h1>
        <el-divider></el-divider>
        <el-card class="filter-bar" shadow="never">
            <el-form :inline="true" :model="filterCondition"  ref="transportFilter">
                <el-form-item label="Receiving User">
                    <el-select v-model="filterCondition.receiverUserId" placeholder="Please Choose Admin in Charge " clearable :style="{width: '100%'}">
                        <el-option v-for="(item, index) in userList" :key="index" :label="item.label" :value="item.value" :disabled="item.disabled"></el-option>
                    </el-select>
                </el-form-item>
<!--                <el-form-item label="Related Request">-->
<!--                    <el-select v-model="filterCondition.status" placeholder="Related Request">-->
<!--                        <el-option label="Order Unsuccessful" value="Order Unsuccessful"></el-option>-->
<!--                    </el-select>-->
<!--                </el-form-item>-->
                <el-form-item>
                    <el-button type="info" size="medium" @click="resetForm('transportFilter', resetFilter)">Clear</el-button>
                    <el-button type="primary" size="medium" @click="submitForm('transportFilter', loadList)">Select</el-button>
                </el-form-item>
            </el-form>
        </el-card>
        <el-table :data="list" border style="width: 100%" :highlight-current-row="true" :stripe="true">
            ...
            <el-table-column  prop="approvalId" label="Related Request ID" width="180" align="center"></el-table-column>
            <el-table-column  prop="createTime" label="Time" width="150" align="center"></el-table-column>
            <el-table-column  prop="expectTime" label="Expected Delivery Time" width="150" align="center"></el-table-column>
            ...
            <el-table-column prop="receiverUserId" label="Receiving User ID" width="150" align="center"></el-table-column>
            <el-table-column prop="transportType" label="Level" width="150" align="center"></el-table-column>
            <el-table-column fixed="right" label="Options" align="center">
                <template slot-scope="scope">
                    <el-button type="text" round size="mini" @click="showMaterial(scope.row.materialRecord)">Check Medical Resources</el-button>
                    <el-divider direction="vertical"></el-divider>
                    <el-button type="text" round size="mini" @click="showTransport(scope.row)">Delivering Records</el-button>
                    <el-divider direction="vertical"></el-divider>
                    <el-popconfirm confirmButtonText='Confirm' cancelButtonText='Cancel' icon="el-icon-info" iconColor="red" title="Are you sure to end delivery?" @confirm="updateStatus(scope.row)">
                        <el-button type="danger" round size="mini" slot="reference" :disabled="isNotIdentity(1)">Delivery Ended</el-button>
                    </el-popconfirm>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-drawer title="运送List：" :visible.sync="showMaterialApplyList" direction="ltr">
        ...
        <el-drawer title="Delivering Status" :visible.sync="showTransportRecord" direction="rtl">
                ...
                <div v-if="transportRecord.length>0">
                    <div  v-for="(r, index) in transportRecord" :key=Date.now()+index style="margin: 10px auto">
                        <h4 style="color: #009fcd">
                            ...
                            {{index+1}}.Delivering Time：<el-tag type="primary" size="small">{{r.time}}</el-tag>
                            <el-divider direction="vertical"></el-divider>
                            <el-button type="text" size="mini" @click="viewLoc(r)">Check Coordinate</el-button>
                        </h4>
                        <p style="padding: 0 8px; font-size: 13px; border-left: lightblue 2px solid; line-height: 20px"><b>Delivering Detail：</b>{{r.address}}</p>
                    </div>
                </div>
                <p v-else  style="text-align: center; margin: 100px 0;"><b>No Delivering Records</b></p>
            </div>
            <div style="text-align: center">
                <el-button type="primary" @click="newTransportVisible=true" :disabled="isNotIdentity(3)">+ New Delivering Status</el-button>
            </div>
        </el-drawer>
        <el-dialog title="Enter new delivering address and coordinate：" :visible.sync="newTransportVisible" width="40%">
            <el-card style="margin: 0 0 10px" shadow="never">
                <el-input v-model="newTransport.loc" readonly style="width: 30%"></el-input>
                <el-divider direction="vertical"></el-divider>
                <el-button @click="cancel" round>Reset</el-button>
                <el-divider direction="vertical"></el-divider>
            ...
            <span slot="footer" class="dialog-footer">
                <el-button type="primary" @click="updateTransport">Confirm</el-button>
              </span>
        </el-dialog>
    </div>
</template>
    ...
    export default {
        ...
        methods:{
            ...
            viewLoc(rec){
                ...
                console.log(rec);
                if(isNull(rec.loc)){
                    this.$alert("This Delivering Status doesn't include coordinates", '提示：',{type: 'info'});
                    return;
                }
            ...
            showMaterial(detail){
                ...
                if (isNull(detail)) {
                    this.$alert("This Delivery doesn't include Order Medical Resource List", '提示：',{type: 'info'});
                    return;
                }
            ...
            updateStatus(record){
                console.log("Delivery Ended Status："+record);
                transportApi.updateTransport({id:record.id, status:'Received'}).then(res=>{
                    this.$message({message:'Update Status Successfully', type:'success'});
                    approvalApi.updateApproval({id:record.approvalId, status:'Delivered'}).then(res=>{
                        this.$message({message:'Order'+record.approvalId+'has been in Delivered Status', type:'success'});
                    });
                });
            },
            async updateTransport(){
                console.log("New Delivering Status");
                ...
                transportApi.updateTransport({id:this.newTransport.id, transportRecord: t}).then(res=>{
                    this.$message({message:'New Delivering Status StatusDone', type:'success'});
                    this.newTransport = {};
            ...
            async loadUser(){
                let res = await userApi.listUser({currentPage: 1, pageSize: 100, identityName:'General User'});
                if(res.status){
                    const list = res.data.list;
                    this.$message({message:"General User Loaded, Amount："+list.length , type: 'success'});
                    list.forEach((v)=>{
                        this.userList.push({
                            'label': v.nickName,
                            'value': v.id
        ...
        mounted(){
            console.log("Load Delivery List");
